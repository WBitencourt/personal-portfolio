...
const toggleTheme = (get: Get, set: Set) => () => {
  ...
  destroyCookie(undefined, 'wbitencourt.current_theme', { domain: `.${window.location.hostname}`, path: '/' });
  setCookie(null, 'wbitencourt.current_theme', theme, { domain: `.${window.location.hostname}`, path: '/' });
}

const startThemeStore = (get: Get, set: Set) => () => {
  try {
    const {'wbitencourt.current_theme': cookieTheme } = parseCookies();
    ...
    destroyCookie(undefined, 'wbitencourt.current_theme', { domain: `.${window.location.hostname}`, path: '/' });
    setCookie(null, 'wbitencourt.current_theme', 'light', { domain: `.${window.location.hostname}`, path: '/' });
  }
}

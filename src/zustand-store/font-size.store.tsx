...
const increaseFontSize = (get: Get, set: Set) => () => {
  ...
  destroyCookie(undefined, 'wbitencourt.font_size', { domain: `.${window.location.hostname}`, path: '/' });
  setCookie(null, 'wbitencourt.font_size', String(newSize), { domain: `.${window.location.hostname}`, path: '/' });
}
...
const startFontSizeStore = (get: Get, set: Set) => () => {
  try {
    const {'wbitencourt.font_size': cookieFontSize } = parseCookies();
    ...
    destroyCookie(undefined, 'wbitencourt.font_size', { domain: `.${window.location.hostname}`, path: '/' });
    setCookie(null, 'wbitencourt.font_size', String(DEFAULT_FONT_SIZE), { domain: `.${window.location.hostname}`, path: '/' });
  } catch {
    set({ fontSize: DEFAULT_FONT_SIZE });
    updateDOMFontSize(DEFAULT_FONT_SIZE);
